...
my_colors2 <- RColorBrewer::brewer.pal(11, "Paired")[c(3,4)]
...
# Table of TUs: Device by participant, separated by platform
...
# Table of tokens sent: Device by participant, separated by platform

disc_tbl <- corpus %>% 
  filter(Platform == "Discord") %>% 
  group_by(Participant, Device) %>% 
  summarise(wordcount = sum(word_count))
write_csv2(disc_tbl, "disc_tbl.csv")
twit_tbl <- corpus %>% 
  filter(Platform == "Twitter") %>% 
  group_by(Participant, Device) %>% 
...
# Graph: Device by participant over platform
ggplot(tbl, aes(y=Message, x=Platform, fill = Device)) + 
...
# This way rounded to nearest time, for Discord
discord_corpus <- corpus %>% filter(Platform == "Twitter")
discord_corpus$time <- round_date(discord_corpus$Date_Time, unit = "month")
discord_corpus <- discord_corpus %>% filter(Participant %in% c("Tereza", "Leila", "Ina", "Roy"))
time_tbl <- discord_corpus %>% 
  group_by(Participant, Device, time) %>% 
  summarise(TUs = n())
ggplot(time_tbl, aes(y=TUs, x=time, fill = Device)) + 
  ...
  theme(axis.title = element_text(face="bold"),
        ...
        legend.title = element_text(size=12),
        legend.text = element_text(size=12))
# Interestingly, no big change for Leila phone-computer on Twitter.
...
ggplot(discord_corpus, aes(x = Participant, fill = Firstletter)) +
